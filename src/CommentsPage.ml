...
type rec state = {story_with_comments: option<StoryData.story_with_comments>}

type rec action = Loaded(StoryData.story_with_comments)

@react.component
let make = (~id) => {
  let (state, dispatch) = React.useReducer((_state, action) => switch action {
        ...
        <span>
          {
            let time = story.time
            let by = story.by
            React.string(` submitted $time by $by`)
          }
        </span>
      </span>
    </div>
  ...
  <div className="CommentsPage_container">
    ...
    | Some(story) =>
      <div> {renderTitle(story)} {renderByline(story)} <CommentList story=story /> </div>
