...
let make = () => {
  let (state, dispatch) = React.useReducer((state, action) => switch action {
    ...
    | Loaded((page, data)) => let updatedTopstories = Array.concat(state.topstories, data)
  ...
  React.useEffect(() => {
    ...
    let loadNextPage = () => if state.page < 4 {
        StoryData.fetchTopStories(state.page, payload => dispatch(Loaded(payload))) |> ignore
        dispatch(Loading)
      }
    let scrollHander = _e => if nearTheBottom() && !state.loading {
        loadNextPage()
      }
  ...
  <div>
    {if Array.length(state.topstories) > 0 {
      state.topstories->Array.mapWithIndex((index, story) =>
        ...
        <StoryListItem key={string_of_int(story.id + index)} index=index story=story />
      )->React.array
    } else {
      ReasonReact.null
    }}
